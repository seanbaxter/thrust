...
__global__ void repro_kernel(ExecutionPolicy exec, Iterator1 first, Iterator1 last, Iterator2 result)
{
...
void Test(const std::size_t n)
{
  ...
  thrust::device_vector<T> d_input(n);
  thrust::device_vector<T> d_output(n);
  repro_kernel<<<1,1>>>(exec, d_input.begin(), d_input.end(), d_output.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
}
DECLARE_VARIABLE_UNITTEST(Test);
...
//  thrust::device_vector<T> d_input = h_input;
  thrust::host_vector<T>   d_input(n);// = unittest::random_samples<T>(n);
